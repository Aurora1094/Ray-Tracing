...
{
    // 全局光照系数
    const double globalLightCoefficient = 0.2;
    ...
    // 环境光
    Vector3 ambient = mixcol * 0.4 + globalLight;
...
Vector3 traceRay(const Ray &ray, const std::vector<Sphere> &spheres, const Light &light, int depth = 0) {
    ...
    // 计算直接光照
    Vector3 directLight = calculatePhongLighting(hitPoint, normal, light, camera, *closestSphere);
...
protected:
    void paintEvent(QPaintEvent *event) override
    {
        ...
        light.color = Vector3(255.0 / 255.0, 200.0 / 255.0, 100.0 / 255.0);
